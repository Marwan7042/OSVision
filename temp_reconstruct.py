...
# Appearance gate: pre-filters loop closure candidates by image similarity
# before committing to expensive FGR + ICP evaluation.
# When enabled, the spatial search radius is expanded (appearance filters false positives).
# Fails open (passes candidate) if image load fails — ICP remains the final arbiter.
_APPEAR_CFG          = CFG["reconstruction"].get("appearance_gate", {})
APPEAR_ENABLED       = bool(_APPEAR_CFG.get("enabled", True))
APPEAR_THR           = float(_APPEAR_CFG.get("similarity_threshold", 0.55))
APPEAR_SPATIAL_MULT  = float(_APPEAR_CFG.get("spatial_gate_multiplier", 3.0))
...
# ============================================================
# APPEARANCE DESCRIPTOR CACHE
# ============================================================
# 32×32 grayscale resize → 1024-d L2-normalised vector.
# Cost: ~0.2ms per frame (cv2.resize + linalg.norm). Zero new dependencies.
# Cosine similarity is a single dot product on normalised vectors.
_desc_cache = PromiseLRU(120)

def _compute_descriptor(index):
    """Load RGB, resize to 32×32 grayscale, L2-normalise. Returns None on failure."""
    img = cv2.imread(rgb_files[index], cv2.IMREAD_GRAYSCALE)
    if img is None:
        return None
    small = cv2.resize(img, (32, 32), interpolation=cv2.INTER_AREA).astype(np.float32).flatten()
    n = np.linalg.norm(small)
    if n < 1e-6:
        return None
    return small / n

def appearance_similar(idx_a, idx_b):
    """Returns True if frames are visually similar enough to attempt ICP.
    
    Fails OPEN (True) if either descriptor is unavailable — ICP remains the
    final geometric arbiter. Only rejects when both descriptors exist and
    cosine similarity is clearly below threshold.
    """
    if not APPEAR_ENABLED:
        return True
    da = _desc_cache.get_or_compute(idx_a, _compute_descriptor)
    db = _desc_cache.get_or_compute(idx_b, _compute_descriptor)
    if da is None or db is None:
        return True
    return float(np.dot(da, db)) >= APPEAR_THR
...
print(f"  Spatial gate : {SPATIAL_GATE:.2f}m × {APPEAR_SPATIAL_MULT if APPEAR_ENABLED else 1.5:.1f} = {SPATIAL_GATE * (APPEAR_SPATIAL_MULT if APPEAR_ENABLED else 1.5):.2f}m")
print(f"  Appear. gate : {'ON (thr=' + str(APPEAR_THR) + ')' if APPEAR_ENABLED else 'OFF'}")
...
with ThreadPoolExecutor(max_workers=_phys) as pool:
    for i in range(MIN_LOOP_FRAME_DISTANCE, n_frames, LOOP_CLOSURE_INTERVAL):
        ...
        # When appearance gate is enabled we expand the spatial search radius
        # (APPEAR_SPATIAL_MULT vs. the old hardcoded 1.5×). Appearance filtering
        # then suppresses false positives before the expensive FGR+ICP step, so
        # the total CPU cost stays comparable while catching drift-separated loops.
        _smult = APPEAR_SPATIAL_MULT if APPEAR_ENABLED else 1.5
        mask = dists <= (SPATIAL_GATE * _smult)
        ...
        for ti in reversed(valid_ti):
            # Appearance gate: reject clearly dissimilar frames before FGR+ICP.
            # This is ~0.4ms (two descriptor lookups + dot product) vs ~200ms for ICP.
            if not appearance_similar(i, ti):
                n_skip += 1
                continue
            n_try += 1
